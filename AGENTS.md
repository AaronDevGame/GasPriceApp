# GasPriceApp Monorepo Agent Guide
...
- `GasPriceApp.sln`: monorepo-level .NET solution.
- `GasPriceBackend/`: existing BackendServer project and backend-specific files.
- `GasPriceBackend/Program.cs`: application composition, middleware ordering, public/admin endpoint mapping, and in-memory server status.
- `GasPriceBackend/Auth.cs`: guest login, logout, token generation, device identity, and player-name handling.
- `GasPriceBackend/PlayerDataEndpoints.cs`: authenticated player-data reads and patches.
- `GasPriceBackend/AppDbContext.cs`: EF Core model configuration and database connection resolution.
- `GasPriceBackend/ServerData.cs`: API contracts, routes, route registries, and API version.
...
```bash
dotnet restore GasPriceApp.sln
dotnet build GasPriceApp.sln
dotnet run --project GasPriceBackend/BackendServer.csproj
...
```bash
dotnet ef migrations add DescriptiveMigrationName --project GasPriceBackend/BackendServer.csproj
```
